import React from 'react';
import { Link } from 'react-router-dom';
import PlaceImg from './PlaceImg';
import axiosInstance from '@/utils/axios';
import { toast } from 'react-toastify';

const InfoCard = ({ place }) => {
  const handleDelete = async () => {
    // Call the onDelete function with the place ID
    try {
      await axiosInstance.delete(`places/delete-place/${place._id}`);
      // Reload the page
      window.location.reload();
    } catch (error) {
      console.log(error);
      // Show an error message Toast
      toast.error('Failed to delete the place');
    }
  };

  return (
    <div className="my-3 flex flex-col gap-4 rounded-2xl bg-gray-100 p-4 transition-all hover:bg-gray-300 md:flex-row">
      <div className="flex w-full shrink-0 bg-gray-300 sm:h-32 sm:w-32 ">
        <PlaceImg place={place} />
      </div>
      <div className="flex flex-grow flex-col justify-between">
        <div>
          <h2 className="text-lg md:text-xl">{place.title}</h2>
          <p className="mt-2 line-clamp-3 text-sm">{place.description}</p>
        </div>
        <div className="mt-4 flex items-center justify-between">
          <Link
            to={`/account/places/${place._id}`}
            className="text-blue-500 hover:underline"
          >
            View Details
          </Link>
          <button
            onClick={handleDelete}
            className="text-red-500 hover:underline"
            style={{ padding: '5px 10px', borderRadius: '20px' }}
          >
            Delete
          </button>
        </div>
      </div>
    </div>
  );
};

export default InfoCard;
